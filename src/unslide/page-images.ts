      ...
      scoped(
        Effect.gen(function* () {
          ...
          const previousNames = yield* Effect.tryPromise({
            try: () =>
              readdir(outputDirectory, { withFileTypes: true }).then((entries) =>
                entries
          ...
          const publication = Effect.gen(function* () {
            ...
            for (const page of generated) {
              ...
              if (
                !/^page-\d+\.png$/.test(fileName) ||
                path.resolve(outputDirectory, fileName) !== path.resolve(page.outputPath)
              ) {
          ...
          const publicationExit = yield* Effect.exit(
            withLogPhase(publication, kind === "captures" ? "captures.publish" : "images.publish", {
          ...
          if (Exit.isFailure(publicationExit)) {
            ...
            yield* logDebug(
              rollbackErrors.length === 0
                ? "publication.rollback.completed"
                : "publication.rollback.incomplete",
              {
              ...
              return yield* Effect.failCause(
                rollbackErrors.length === 0
                  ...
                  : Cause.combine(
                      publicationExit.cause,
                      Cause.fail(
                        commandFailure(publicationExit.cause, context, publicationMessage),
                      ),
                    ),
              );
            }
