...
The initial reports proved explicit pages, standalone HTML, and isolated
browser capture. Their repository-local foundation also fixed A4 landscape
geometry, content insets, header and footer regions, numbering, CSS class names,
...
- Validation must distinguish protocol failures from authoring mistakes without
  repairing either.
- Full document and asset ownership makes the authoring module deeper than the
  original string writer.
- Source recipe upgrades cannot be treated like normal package upgrades after a
  user edits installed files.

## Rejected Alternatives

- Keep copying the repository-local visual foundation. This distributes
  maintenance and prevents centralized fixes.
- Publish that page foundation unchanged. This makes one report design part of
